...
```{r}
plot_rating <- ggplot() + geom_bar_interactive(speaker$rating_melt, mapping = aes(x = ..count../sum(..count..)*5, y = speaker$rating_melt$Category, fill = speaker$rating_melt$Rating), alpha = .8, colour = "white", size = 4, width = .6) +
...
ggiraph(code = {print(plot_rating)}, width_svg = 9.5, height_svg = 4.8 ) 
```

#### What attendees liked about the speech and their suggestions:

```{r}
row.names(speaker$evaluation) <- NULL
speaker$evaluation[is.na(speaker$evaluation)] <- ""
formattable::format_table(speaker$evaluation, align = "l")
```